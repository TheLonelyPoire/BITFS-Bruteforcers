...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
            ...
            point1[0] = std::stof(argv[i + 1]);
            point1[1] = std::stof(argv[i + 2]);
            point2[0] = std::stof(argv[i + 3]);
            point2[1] = std::stof(argv[i + 4]);
            point3[0] = std::stof(argv[i + 5]);
            point3[1] = std::stof(argv[i + 6]);
            point4[0] = std::stof(argv[i + 7]);
            point4[1] = std::stof(argv[i + 8]);
